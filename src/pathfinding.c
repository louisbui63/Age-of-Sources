...
#define pqueue_push_wrapper(p, ind, from, w)                                   \
  (p = pqueue_push_wrapper_inner(p, ind, from, w))
...
#define flatmap_index(m, i, j)                                                 \
  (1 + (i) % ((int *)(m))[0] + (j) * ((int *)(m))[0])

// converts a TilePosition into flatmap coordinates
#define flatmap_tile_index(m, t)                                               \
  (1 + ((t)->x) % ((int *)(m))[0] + ((t)->y) * ((int *)(m))[0])
